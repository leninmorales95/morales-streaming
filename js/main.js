...
const detailPayButton = document.getElementById("detailPayButton");
...
if (detailAddCart) {
  detailAddCart.addEventListener("click", () => {
    ...
    closeDetail();
    agregarAlCarrito(currentDetailProduct.name, currentDetailProduct.price);
  });
}

if (detailPayButton) {
  detailPayButton.addEventListener("click", () => {
    closeDetail();
    openPaymentModal("yape", "product");
  });
}
...
function procesarDatosDeLaWeb(data) {
  ...
  if (listaProductos.length > 0) {
    ...
    const productCards = rawProductosData.map(p => {
      ...
      const miniBadgeHtml = (p.badge_mini && !isSoldOut)
        ? `<span class="card-mini-badge">${p.badge_mini}</span>`
        : "";
...
function selectPayMethod(method, btnElement) {
  selectedPayMethod = method;
  document.querySelectorAll(".pay-option-btn").forEach(b => {
    b.style.borderColor = "rgba(255,255,255,0.1)";
    b.style.background = "rgba(255,255,255,0.03)";
    b.style.color = "var(--text-gray)";
  });
  btnElement.style.borderColor = "rgba(0,212,255,0.4)";
  btnElement.style.background = "rgba(0,212,255,0.15)";
...
function updateModalWhatsappLink() {
  ...
  if (priceElement) {
    const fullPriceText = priceElement.textContent;
    const matches = fullPriceText.match(/S\/\d+(\.\d+)?/g);
    cleanPrice = matches ? matches[matches.length - 1] : fullPriceText.trim();
  }

  const btn = document.getElementById("detailWhatsapp");
  let customWaMsg = `🐱‍👤 ¡Hola! Vengo de la página web 🌐\n\n🎬 ¡Deseo activar mi servicio de *${name}*! (${cleanPrice})\n💳 *Método de pago elegido:* ${selectedPayMethod}\n\n¿Me pasas los datos para realizar el pago por favor?`;
  btn.href = createWhatsappLink(customWaMsg);
}
